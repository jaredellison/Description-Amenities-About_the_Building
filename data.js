...
let highlightsCreator = (divider, HLAArray) => {
  ...
  // console.log('newHLArray',newHLArray);
  return newHLArray;
};
...
var descriptionGenerator = () => {
  let paragraphCount = Math.floor(9 * Math.random()) + 2;
  return Array(paragraphCount)
    .fill()
    .map(() => {
      let paragraphLength = Math.floor(9 * Math.random()) + 2;
      return faker.lorem.paragraph(paragraphLength);
    })
    .join('\n \n');
};

dbComponents.db.once('open', function() {
  console.log('Saving to database');
  dbComponents.saver(
    descriptionGenerator(),
    highlightsCreator,
    highlightAmenities,
    buildingAmensCreator,
    buildingAmenities,
    listingAmensCreator,
    listingAmenities,
    outdoorAmensCreator,
